...
errno_t
hbac_evaluate_time_rules(struct hbac_time_rules **rules,
                         time_t req_time,
                         bool *matched)
{
    errno_t ret = EOK;
    ...
    int j;  /* iterator of an array of icalendar strings */
            /* contained in a timerule structure */
    ...
    for (i = 0; rules[i]; i++) {
        for (j = 0; rules[i]->accesstimes[j]; j++) {
            ret = eval_time_rule(rules[i]->accesstimes[j], now, matched);
            if (ret != EOK) {
                *matched = false;
                ret = ENOMATCH;
                goto done;
...
char *get_olson_timezone(void)
{
    ...
    if (S_ISLNK(file_stat.st_mode)) {
         ...
         */
        if((bytes_read = readlink("/etc/localtime", path, PATH_MAX)) == -1)
        {
            goto err;
        }
    }
        ...
        if ((bytes_read = fread(path, sizeof(char), PATH_MAX, tzfile)) == 0 ||
            ferror(tzfile))
        {
    ...
    /* null-terminate the string for strtok */
    path[bytes_read] = 0;
    token = strtok(path, "/");
